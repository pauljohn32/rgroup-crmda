...
dat2n <- colnames(dat2)
for(i in dat2n){print(i); nname <- paste0(as.character(i),"c"); dat2[[nname]] <- drop(scale(dat2[[i]]))}
...
plotPlane(m4mc, plotx1="x1c", plotx2="x2c", plotPoints=FALSE, main="Mean-centered", ticktype="detailed")
...
m4mcpred <- fitted(m4int)#predict(m4mc, newdata=dat2)
m4rcpred <- fitted(m4rc) #predict(m4rc, newdata=dat2)
m4intpred <- fitted(m4int) #predict(m4int, newdata=dat2)
op <- par(no.readonly = TRUE)
par(mfcol=c(1,2))
plot(fitted(m4rc), predict(m4rc, newdata=dat2))
plot(fitted(m4mc), predict(m4mc, newdata=dat2))
