...
## jntest= Johnson-Neyman technique for probing interactions and finding regions of significance
## j a function that takes these arguments:
## model <- m1 #object
...
jntest <- function(model=NULL, plotx=NULL,modx=NULL, depVar=NULL){
  if (is.null(model)) stop("jntest requires a fitted regression model.")
  if (is.null(plotx)) stop("jntest requires the name of the variable to be drawn on the x axis")
  if (is.null(modx)) stop("jntest requires the name of moderator variable for which several slopes are to be drawn")
  ##grab model matrix
  mm <- model.matrix(model)
  if (is.null(depVar)){
    if (is.null(model$y)) stop("jntest must have the dependent variable. You must either supply depVar as an argument or create your lm model with the argument y=T so that jntest can grab the dependent variable from the fitted model.")
     else depVar <- model$y
  }
