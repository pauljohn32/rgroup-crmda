...
## R's default random generator is MT19937.  Because most people don't
## seem to understand the difference between a seed value and the
## internal state of the random generator, I've prepared this little
## example program.  This illustrates the internal state of
## MT19937 and discusses some ways to re-set that generator to
...
## It is especially easy to generate random numbers from a uniform distribution. All we  have to do is take the randomly drawn positive integer (which the generator provides, because that's what generators do) and divide by the largest possible integer.

runif(1)

## What is the internal state of the generator?  I literally mean,
## what numbers summarize the machine that can generate random
## bumbers. Run this to see. Don't hate me if it fills up a page.
...
## That's R's indicator for the type of the generator. Unless
## something has changed dramatically, 403 refers to the default
## random generator in R (and most good programs, actually), which is
## MT19937.
...
## That is a marker peculiar to the way MT19937 works.  It indicates
## that this  particular vector has been used to draw one random number.
...
## place a value into an environment.
