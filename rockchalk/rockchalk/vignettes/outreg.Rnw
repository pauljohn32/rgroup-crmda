...
\title{Using rockchalk Functions for Quick \& Consistent Regression Presentations}
...
plot. The idea is to consider the effect of $x1$ on $y$ for several
values of $x2$, and then possibly test whether the plotted effects
...
<<ps15>>=
dat$y4 <- with(dat, -3*x1 + 4*x2 - 0.17*x1*x2 + 5*rnorm(100))
...
The ``simple slope'' lines for the interactive model are presented
in Figure \ref{fig:ps20}.
...
along with some diagnostic plots, as illustrated in figure \ref{fig:ts10}.
...
The plotPlane function offers another way to visualize the mutual
effect of two predictors in a regression model. See Figure \ref{fig:pp10}

\begin{figure}


\caption{plotPlane for the Interactive Model\label{fig:pp10}}


<<pp10, fig=T>>=
p10 <- plotPlane(m4, plotx1="x1", plotx2="x2")
@
\end{figure}
...
of what that might look like is presented in Figure \ref{fig:pp20}

\begin{figure}


\caption{A Planned Enhancement of plotPlane\label{fig:pp20}}


<<pp20, fig=T, height=5>>=
par(mfcol=c(1,2))
m4ps <- plotSlopes(m4, plotx="x1", modx="x2", xlab="Continuous Predictor", ylim=c(-25, 35))
p20 <- plotPlane(m4, plotx1="x1", plotx2="x2")
for(j in unique(m4ps$newdata$x2)){
subdat <- m4ps$newdata[m4ps$newdata$x2==j,]
lines(trans3d(subdat$x1, subdat$x2, subdat$pred, pmat=p20$res), col="red", lwd=3)
}
@
...
If there is a nonlinearity, for example an interaction, then it is
a little more fun. I create a new dependent variable y4 and then put
