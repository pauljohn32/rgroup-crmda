...
In order to create more workable output objects, some re-arrangment
...
cases we find interesting. 

My preferred approach is as follows. Set most of the predictors at
their central values (mean or mode). Then specify some alternate levels
for particular predictors. In the context of model m1, suppose we
want to formulate predictions for each of the 3 middle quantiles of
the age variable and for the levels of region called ``C'' and ``N''.
The summarize function and the centralValues functions are run first
to collect up the needed information.
...
the mixAndMatch data frame with the central values in m1cv. It is
important to be cautious about creating columns that have the same
names when combining them, however. In this example, the cbind function
is used to join the results into a data frame, but only after the
``overlapping'' column names are removed from m1cv (literally, this
keeps only the columns in m1cv which are not also in mixAndMatch). 
...
\begin{equation}
\hat{y}_{i}=\hat{b}_{0}+\hat{b}_{1}x1_{i}+\hat{b}_{2}x2_{i}+\hat{b}_{3}x1_{i}x2_{i}+x3_{i}.\label{eq:plotSl10}
\end{equation}
...
regression model, and then we pass that object to plotSlopes. For
example, suppose we the model in equation \ref{eq:plotSl10}, and
then plot the effect of $x1$ as it depends on $x2$. 
...
be the horizontal axis, and x2 serves as the moderator variable. plotSlopes
requires that the plotx argument must be the name of a numeric variable,
but modx may be the name of either a numeric or a factor variable. 
...
plotSlopes is intended for linear models, or linear models with multiplicative
interactions. It returns an object that may facilitate further analysis.
The output object includes the newdata object that was used to construct
the plots that are draw. That output object is used by the function
testSlopes, which is discussed below.
...
plotSlopes and plotCurves are intended to speed up the process of
visualization in regression analysis. The difference between them
is that plotSlopes is confined to more-or-less linear models, wherease
plotCurves is intended to be a bit more general. plotCurves allows
nonlinear terms in the regression model that is being plotted. In
that sense, plotCurves is rather similar to R's own termplot function.
The difference is that plotCurves allows for moderator variables. 

Suppose a dependent variable y5 is created according to a nonlinear
process.
\begin{equation}
y5_{i}=-3x1_{i}+1.5*log(x2^{2})+1.1x2_{i}+2.2x1_{i}\times x2_{i}+e_{i}
\end{equation}


<<>>=
dat$y5 <- with(dat, -3*x1 + 3.5*log(x2^2) + 2.1*x2 + 2.2 *x1 * x2 + 20*rnorm(100)) 
...
are set to central reference values. As illustrated in Figure \ref{fig:pcps20},
...
value of ``modx,'' the moderator variable. The user should first
run plotSlopes, and then submit the output object to testSlopes. The
usual use case would be the following:
...
is useful, but calculating it was interesting. 
...
result object is passed ot residualCenter like so:
...
in regression analysis has become quite widely established. As a result,
it is necessary to re-trace some steps to find out how we arrived
in our current situation.

For this example, I used the function genCorrelatedData in rockchalk.
The ``true model'' from which the data is produced is
\begin{equation}
...
consider the first two columns of Table \ref{tab:meancenter10-1}.
...
an interaction term. That interaction model, the product variables
$x1\times x2$, is estimated in R with
...
\noindent This is where the ``what the heck?'' or ``Holy Cow!''
response occurs. The regression went to hell! Neither of the key variables,
$x1$ nor $x2$, is ``statistically significant'' any more. While
the coefficients for the variables $x1$ and $x2$ did seem to be
substantial in the first model, the introduction of the interactive
effect appears to ruin the whole thing. What should be done when adding
a product term seems to ``ruin'' a regression model? 
...
The primary advocates of ``mean-centering'' have been Aiken and
West (1991), who then integrated that advice into the very widely
used regression textbook, \emph{Applied Multiple Regression/Correlation
for the Behavioral Sciences} (Cohen, et. al , 2002). Cohen, et al.
refer to the apparent instability of the coefficients as a reflection
of ``inessential collinearity'' among the predictors, due to the
fact that $x1$ and $x2$ are correlated with the new terms, $x2^{2}$
or $x1\cdot x2$. They advised their readers to ``mean center''
their predictors, to subtract the mean of each predictor from the
observed model and run the regression again. 
...
the practice. The ordinary model and the mean-centered models are
actually exactly the same in every important way. The first hint is
in the fact that the coefficient of the interactive effect in columns
2 and 3 is identical. The root mean square and $R^{2}$ estimates
are identical. 
...
we see that the models produce identical predicted values! 
...
With the original, non-transformed data, the vertical (y) axis is
positioned at the front-left edge of the graph, while the centered
one re-positions the y axis into the center of the graph. The coefficient
estimates from a regression model on a nonlinear surface depend on
the location of the y axis. At some points, the estimates will be
large, at some points they will be small. Mean-centering may accidentally
reposition the axis to a location that has ``better'' point estimates.
...
Where mean-centering seems to reduce the damage done by inessential
collinearity, residual-centering completely eliminates it. Because
the residual-centered interaction variable is, by definition, completely
uncorrelated with the other variables in the model, the problem of
collinearity seems to be completely solved. 
...
and the residual-centered models are exactly the same. The calculations
clashed with our intuition. Perhaps the most persuasive case is found
in Figure \ref{fig:rcenter40}.

\begin{figure}
<<rcenter40, fig=T,echo=FALSE, height=3.5, width=7>>=
...
is used as the ``residual-centered'' predictor in place of $(x1_{i}\times x2_{i})$
in equation \ref{eq:int}. 
...
Aiken, L. S. (1991). Multiple Regression: Testing and Interpreting
...
Collinearity Problems in Moderated Multiple Regression \emph{Models.
Marketing Science}, 26(3), 438\textendash{}445. doi: 10.1287/mksc.1060.0263
...
Psychological Measurement}, 58(1), 42 \textendash{}67. doi:10.1177/0013164498058001005
\end{document}
