...
##' Add seeds for more runs to a portableSeeds object
##'
##' After seedCreator has created a collection of seeds,
##' sometimes it is necessary to conduct additional runs that
##' require more initializing values. This function fulfills
##' that need.
##' @param projSeeds A portableSeeds object created by seedCreator
...
##' @author Paul E. Johnson <pauljohn@@ku.edu>
##'
update.portableSeeds <- function(projSeeds, more, file = NULL){
    require(parallel)
    RNGkind("L'Ecuyer-CMRG")

    if ( (missing(projSeeds)) | (!class(projSeeds)== "portableSeeds") ) {
    ...
    nReps <- length(projSeeds)
    streamsPerRep <- length(projSeeds[[1]])

    for (i in nReps:(nReps + more)) {
        for (j in 1:streamsPerRep){
            projSeeds[[i]][[j]] <- s
            s <- nextRNGStream(s)
        }
