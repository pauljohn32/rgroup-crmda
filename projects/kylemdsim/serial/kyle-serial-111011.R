...
fitMissinModels <- function(dat, parms)
    {
      ...
      fitRawMods <- function(x, parms)
        {
          ...
          list(rawNobs=list(rawFull=nobs(fullMod),rawRes=nobs(resMod)),

               rawFullOut=list(rawFullFit=fitMeasures(fullMod), rawFullCoef=coef(fullMod), rawFullSE=inspect(fullMod, "se"), rawFullDx=inspect(fullMod, "dx"), rawFullLL=logLik(fullMod)),
      ...
      list(smMissOut=list(smNobs=list(smFull=nobs(smFullMod),smRes=nobs(smResMod)),

             smFullOut=list(smFullFit=fitMeasures(smFullMod), smFullCoef=coef(smFullMod), smFullSE=inspect(smFullMod, "se"), smMissDx=inspect(smFullMod, "dx"), smFullLL=logLik(smFullMod)),
           ...
           rawMissOut=rawMissOut)
             ...
             #rawMissOut=list(rawFullOut=list(rawFullFit=fitMeasures(rawMissOut[[1]]), rawFullCoef=coef(rawMissOut[[1]]), rawFullSE=inspect(rawMissOut[[1]], "se"), rawFullDx=inspect(rawMissOut[[1]], "dx"), rawFullLL=logLik(rawMissOut[[1]])),

               #rawResOut=list(rawResFit=fitMeasures(rawMissOut[[2]]), rawResCoef=coef(rawMissOut[[2]]), rawResSE=inspect(rawMissOut[[2]], "se"), rawResDx=inspect(rawMissOut[[2]], "dx"), rawResLL=logLik(rawMissOut[[2]]))))
...
fitControlModel <- function(dat4,parms4)
    {
      ...
      list(conNobs=list(full=nobs(fullMod),res=nobs(resMod)),

           conFullOut=list(conFullFit=fitMeasures(fullMod), conFullCoef=coef(fullMod), conFullSE=inspect(fullMod, "se"), conMissDx=inspect(fullMod, "dx"), conFullLL=logLik(fullMod)),
...
runMissSim <- function(dat3, pm, parms3)
    {
      ...
      label <- paste("cellMissingOut-PM:",pm,sep="")
...
#Rprof("goBabyGoProf.out",memory.profiling=TRUE) # Profiles the main function

goBabyGo(rp, parms=parameters)

load("missOut-run-1-omit-10-5743.30828385428.RData")

#Rprof(NULL)

#summaryRprof("goBabyGoProf.out")
...
#data <- simData(parms=parameters)

#miss.dat <- makeMAR(.20,dat=data,parms=parameters)
...
#freshDat <- simData(parms=ps)

#missDat <- makeMAR(pm=.2,dat=freshDat,parms=ps)

#sum(is.na(missDat))/(dim(freshDat)[1]*ps$lenScale)

#impDat <- imputeStack(dat=missDat,parms=ps)
    
#missOut <- fitMissinModels(dat=impDat, parms=ps)
   
#conOut <- fitControlModel(dat4=freshDat,parms4=ps)

#bigMissOut <- runMissSim(dat3=missDat, parms3=ps)

#rowOut <- rowTask(10, dat=freshDat, runNumber=rp, parms=ps)

#repOut <- repTask(rp, parms2=ps)

load("conOut-run-1-omit-10-1698.07139551267.RData")
